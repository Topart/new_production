	...
	$j(document).ready(function() {
		...
		$j(".product-options input").click(
			...
			function()
			{
				...
				if ( $j(this).parent().parent().parent().parent().attr('class') == 'size' )
				{
					...
					// If canvas borders is selected with black or mirrored border, display the size minus 4"
					// The original size is only needed for correct pricing computation

					// This is now properly taken car of in the backend directly.
					/*
					if (selected_borders == "2\" Black Border " || selected_borders == "2\" Mirrored Border ")
					{
						selected_width = selected_width - 4;
						selected_length = selected_length - 4;
					}
					*/
		...
		*/
		...
		$j(".postWrapper").hover(	
			...
			function () {
				$j(this).siblings(".postWrapper").stop(true).animate({ opacity: 0.3 }, "fast");
			}, 
			
			function () {
				$j(this).siblings(".postWrapper").animate({ opacity: 1 }, "fast");
			}
			
		);
		
		
		$j(".blog-index-index .postContent p img").click(function () {
			window.location = $j(this).parent().next().attr("href");
		});
		
		$j(".blog-index-index .postContent p").click(function () {
			window.location = $j(this).next().attr("href");
		});
		
		$j(".blog-cat-view .postContent p img").click(function () {
			window.location = $j(this).parent().nextAll("a").attr("href");
		});
		
		$j(".blog-cat-view .postContent p").click(function () {
			window.location = $j(this).nextAll("a").attr("href");
		});
		...
		// Display the correct delivery time
		var shipping_options = $j(".selected_filter tr").siblings();
		$j( shipping_options ).each(

			function() 
			{
				var framing_option = $j(this).find("dl.item-options dt:contains('Frame')").next().html();
				//alert(framing_option);
				if (framing_option.indexOf("No Frame") == -1)
				{
					$j("#shipping_no_frame").hide();
					$j("#shipping_frame").show();
		...
		function get_current_customization()
		{
			// Material
			var current_material = $j(".current_material").html();

			// Size
			var current_size = $j(".current_size").html();

			// Borders
			var current_borders = $j(".current_borders").html();

			// Canvas Stretching
			var current_canvas_stretching = $j(".current_canvas_stretching").html();

			// Frame
			var current_frame = $j(".current_frame").html();
	
			// Mats
			var current_mats = $j(".current_mats").html();

			var current_customization = encodeURIComponent("material=" + current_material + "&size=" + current_size + "&borders=" + current_borders + "&canvas_stretching=" + canvas_stretching + "&frame=" + current_frame + "&mats=" + current_mats);
			
			//alert(current_customization);

			// Update the product options URL for social sharing
			// Get the names of the currently selected custom options
			var new_facebook_url = $j("#facebook_url").attr('onclick').replace('product_options', current_customization);
			$j("#facebook_url").attr('onclick', new_facebook_url);

			var new_twitter_url = $j("#twitter_url").attr('onclick').replace('product_options', current_customization);
			$j("#twitter_url").attr('onclick', new_twitter_url);

			var new_pinterest_url = $j("#pinterest_url").attr('href').replace('product_options', current_customization);
			$j("#pinterest_url").attr('href', new_pinterest_url);
		}


		$j(".facebook_share.social_bar_icon").click(

			function()
			{
				get_current_customization();
			}
		);
		...
		// Automatically select the "Professional" group in the pro signup page
		$j(".cms-pro-signup #customer_registration_group select#group_id option:contains('Professional')").attr("selected", "selected");
		$j(".cms-pro-signup #customer_registration_group").hide();

		$j(".customer-account-create #customer_registration_group select#group_id option:contains('General')").attr("selected", "selected");
		$j(".customer-account-create #customer_registration_group").hide();



		$j("#our_best_sellers li img").hover(
		
			function () {
				$j(this).css("opacity", 0.3);
				$j("#our_best_sellers .show_price_area").show();
			}, 
			
			function () {
				$j(this).css("opacity", 1);
				$j("#our_best_sellers .show_price_area").hide();
			}
			
		);

		$j("#artists_collection li img").hover(
		
			function () {
				$j(this).css("opacity", 0.3);
				$j("#artists_collection .category_name").show();
			}, 
			
			function () {
				$j(this).css("opacity", 1);
				$j("#artists_collection .category_name").hide();
			}
			
		);

		$j(".coupon_label .link").click(
			function()
			{
				$j(".cart .discount, .cart #discount-coupon-form").toggle();
			}
		);


		$j("input#co-shipping-method-form input").trigger("click");

		if (jQuery.browser.mobile)
		{
			$j(".catalog-category-view").css("left", "65px");
		}


		$j(".customer-name input[type=text]").focus(
	    ...
	    $j(".a-desk-widget.a-desk-widget-chat").css("height", "22px");
