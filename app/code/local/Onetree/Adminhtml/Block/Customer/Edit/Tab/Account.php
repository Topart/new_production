 ...
 */
class Onetree_Adminhtml_Block_Customer_Edit_Tab_Account extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Initialize block
     */
    ...
    public function initForm()
    {
        ...
        foreach ($attributes as $attribute) {
            /*if(in_array($attribute->getName(), array("twitteraccount","pinterestaccount"))){*/
                /* 
                 * if customer group not is 5 (pro)
                 * remove twitter and pinterest fields from form
                 */
               /* if($customer->getGroupId() == 5){
                    //$attribute->setIsVisible(false);
                    $ac = $attribute->getAttributeCode();
                    $element = $form->getElement($ac);
                    if ($element) {
        ...
        if($customer->getGroupId() != 5){
            foreach(array("twitteraccount","pinterestaccount") as $ProField){
                $element = $form->getElement($ProField);
                if ($element) {
                    $id = $element->getId();
